...
class FederatedLearningManager:
    ...
    def client_update(self, model, client_data, noise_scale):
        ...
        # Add Laplace noise to the training data
        X_train_noisy = self.add_laplace_noise(X_train, noise_scale)

        history = model.fit(X_train_noisy, y_train, epochs=self.config['local_epochs'], 
    ...
    def plot_progress(self, history, iteration, client_fraction, local_epochs, noise_scale):
        plt.figure(figsize=(12, 10))
        plt.suptitle(f'Client Fraction: {client_fraction}, Local Epochs: {local_epochs}, Noise Scale: {noise_scale}', fontsize=14)
    ...
    def federated_training(self):
        ...
        for iteration in range(self.config['communication_rounds']):
            ...
            for client_id in selected_clients_indices:
                ...
                # Pass the noise_scale parameter from the config to client_update
                noise_scale = self.config.get('noise_scale', 0)  # Default to 0 if not set
                client_weights, train_loss, train_accuracy, val_loss, val_accuracy, num_samples = self.client_update(local_model, self.client_train_val_data[client_id], noise_scale)
            ...
            if (iteration + 1) % self.config['save_interval'] == 0:
                ...
                self.plot_progress(global_history, iteration, self.config['client_fraction'], self.config['local_epochs'], self.config['noise_scale'])
...
client_fractions = [0.1]
local_epoch_values = [20]
noise_scales = [0.1, 0.25, 0.5, 1.0, 1.5, 2.0]

# Dictionary to store final results for each setting
final_results = {}

# Loop over each client fraction, local epoch value, and noise scale
for fraction in client_fractions:
    for local_epochs in local_epoch_values:
        for b in noise_scales:
            ...
            config['noise_scale'] = b

            # Update directories to include client fraction, local epochs, and noise scale
            fraction_epoch_noise_dir = f'fraction_{fraction}_epochs_{local_epochs}_noise_{b}'
            config['model_save_dir'] = os.path.join('./saved_models', fraction_epoch_noise_dir)
            config['plot_save_dir'] = os.path.join('./training_plots', fraction_epoch_noise_dir)

            # Ensure directories exist
            os.makedirs(config['model_save_dir'], exist_ok=True)
            os.makedirs(config['plot_save_dir'], exist_ok=True)

            # Initialize and run federated learning with noise
            fl_manager = FederatedLearningManager(config)
            global_model, training_history = fl_manager.federated_training()

            # Store the final results
            final_results[(fraction, local_epochs, b)] = {
                'model': global_model,
